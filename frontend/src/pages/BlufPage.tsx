  ...
  return (
    <div className="bluf-page">
      ...
      <div className="bluf-page__body">
        <div className="bluf-page__content">
          ...
          <section className="bluf-section">
            ...
            <p className="bluf-section__body">
              ...
              fast any one step runs. This app tracks a single part's journey — it doesn't
              model order volume or how many units are in process at once, so there's no
              number to show you here. But it's the reason a "fast" process can still have a
              terrible lead time: queues compound faster than intuition expects. If the wait
              segments below are large, that's very likely Little's Law showing up in your own
              value stream, not a fluke.
            </p>
          </section>
                ...
                <tbody>
                  {metrics.wait_contributors.map((w, i) => (
                    <tr key={w.edge_id}>
                      ...
                      <td className="bluf-table__duration">
                        {formatDuration(w.wait_time_sec)}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <section className="bluf-section">
            ...
            {!health?.ai_configured && (
              <p className="bluf-section__body bluf-section__body--muted">
                AI is not configured for this instance — everything above is computed without
                it. Set <code>AI_PROVIDER</code> to enable a written narrative here too.
              </p>
            )}
        ...
        <MapChatPanel mapId={mapId} aiConfigured={health?.ai_configured ?? false} />
      </div>
    </div>
  )
}
